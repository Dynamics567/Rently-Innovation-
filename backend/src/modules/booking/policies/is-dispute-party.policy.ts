...
/** Resolves dispute -> bookingId -> booking -> listing -> provider, one hop further than IsBookingPartyPolicy. */
@Injectable()
export class IsDisputePartyPolicy implements PolicyHandler {
  constructor(
  ...
  async handle(context: ExecutionContext): Promise<boolean> {
    ...
    const dispute = await this.disputeService.findByIdOrFail(request.params.id).catch(() => null);
